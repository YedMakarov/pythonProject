...
# Добавлено для задачи "Регистрация покупателей"
...
async def set_username(message, state):
    await state.update_data(msg_username=message.text)
...
async def set_email(message, state):
    await state.update_data(msg_email=message.text)
...
async def set_age(message, state):
    ...
    add_user(data['msg_username'], data['msg_email'], int(data['msg_age']))
